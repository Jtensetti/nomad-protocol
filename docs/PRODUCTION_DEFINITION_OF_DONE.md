...
| PROD-05 | Mix committee keys are created with authenticated distributed key generation and threshold decryption; no machine ever holds a complete long-term decryption key. | At least five independently administered nodes, DKG transcript, threshold-decryption tests and compromise drill. | NOT_MET |
...
| PROD-08 | Committee membership, epoch rotation, forward secrecy, key erasure and compromise recovery are specified and exercised. | Rotation/erasure tests and a completed key-compromise recovery drill. | NOT_MET |
